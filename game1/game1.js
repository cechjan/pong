...
function init() {
    ...
    balls[0] = new Ball(canvas.width / 2, canvas.height / 2);
    randomY = Math.random();
    if(Math.round(randomY) == 0)
        balls[0].yVelocity = balls[0].yVelocity;
    if(Math.round(randomY) == 1)
        balls[0].yVelocity = -balls[0].yVelocity;
    randomX = Math.random();
    if(Math.round(randomX) == 0)
        balls[0].xVelocity = balls[0].xVelocity;
...
function movement(keys) {
    ...
    for(let i = 0; i < balls.length; i++) {
        ...
        repaint();
        if (balls[i].y < 0 && balls[i].yVelocity < 0) {
            balls[i].yVelocity = -balls[i].yVelocity;
            repaint();
        }
        if (balls[i].y > canvas.height && balls[i].yVelocity > 0) {
            balls[i].yVelocity = -balls[i].yVelocity;
            repaint();
        }
        if(balls[i].x < paddleWidth) {
            if(balls[i].y >= (paddle1.y - balls[i].widthHeight + 12) && balls[i].y <= (paddle1.y + paddleHeight + balls[i].widthHeight + 2)){
                balls[i].xVelocity = -balls[i].xVelocity;
                //randomY = Math.random();
                //if(Math.round(randomY) == 0)
                    //dy = ball.y - (paddle1.y + paddleHeight);
                if(balls[i].y + balls[i].widthHeight / 2 > (paddle1.y + (paddleHeight / 2)))
                    balls[i].dy = 20;
                //if(Math.round(randomY) == 1)
                    //dy = - ball.y + (paddle1.y + paddleHeight);
                if(balls[i].y - balls[i].widthHeight / 2 <= (paddle1.y + (paddleHeight / 2)))
                    balls[i].dy = -20;
                balls[i].yVelocity = balls[i].dy * 0.1;
                repaint();
            } else {
                if(score2End == false)
                    score2++;
        ...
        if(balls[i].x > canvas.width - paddleWidth) {
            if(balls[i].y >= (paddle2.y - balls[i].widthHeight + 12) && balls[i].y <= (paddle2.y + paddleHeight + balls[i].widthHeight + 2)){
                balls[i].xVelocity = -balls[i].xVelocity;
                //randomY = Math.random();
                //if(Math.round(randomY) == 0)    
                    //dy = ball.y - (paddle2.y + paddleHeight);
                if(balls[i].y + balls[i].widthHeight / 2 > (paddle2.y + (paddleHeight / 2)))
                    balls[i].dy = 20;
                //if(Math.round(randomY) == 1)
                    //dy = - ball.y + (paddle2.y + paddleHeight);
                if(balls[i].y - balls[i].widthHeight / 2 <= (paddle2.y + (paddleHeight / 2)))
...
function repaint() {
    ...
    if(score2End == false && score1End == false) {
    for(let i = 0; i < balls.length; i++)
        ctx.fillRect(balls[i].x - balls[i].widthHeight, balls[i].y - balls[i].widthHeight, balls[i].widthHeight, balls[i].widthHeight);
    }
    ctx.fillText(score1, ((canvas.width / 2) / 2) - 40, paddleHeight);
    ctx.fillText(score2, canvas.width - ((canvas.width / 2) / 2), paddleHeight);
...
function screenCheck() {
    setInterval(function() {
        ...
        if(score2End == false && score1End == false) {
            ballSpawn++;
            if(ballSpawn % ballSpawnTime == 0) {
                balls[balls.length] = new Ball(canvas.width / 2, canvas.height / 2);
                balls[balls.length - 1].xVelocity = balls[0].xVelocity;
                balls[balls.length - 1].yVelocity = balls[0].yVelocity;
            }
        }
    }, 20);
...
function settingsMenu() {
    settingsInterval = setInterval(function() {
        ...
        textBallSpeedNumCenter = ((canvas.width / 2) / (settingsOutline / 2)) * (settingsOutline / 2 - 1) - ((ctx.measureText(textBallSpeed).width - 
                                                                                                                ...
                                                                                                                //console.log(textPaddleSpeedNumCenter);
        ctx.fillStyle = 'white';
        if(balls[0].xVelocity >= 0)
            ctx.fillText(balls[0].xVelocity, canvas.width / settingsOutline + textPaddleSpeedNumCenter, canvas.height / settingsOutline + 345);
        if(balls[0].xVelocity < 0)
            ctx.fillText(-(balls[0].xVelocity), canvas.width / settingsOutline + textPaddleSpeedNumCenter, canvas.height / settingsOutline + 345);
        ctx.font = "20px addl";
        ctx.fillStyle = 'rgb(105, 105, 105)';
...
function checkClick(event) {
    ...
    if((mouseY > canvas.height / settingsOutline + 300 && mouseY < canvas.height / settingsOutline + 320) && (mouseX > canvas.width / settingsOutline + textBallSpeedNumCenter + 63 && mouseX < canvas.width / settingsOutline + textBallSpeedNumCenter + 83)) {
        for(let o = 0; o < balls.length; o++) {
            if(balls[o].xVelocity >= 0) {
                if(balls[o].xVelocity < 16) {
                    balls[o].xVelocity ++;
                    balls[o].yVelocity = balls[o].xVelocity / 2;
                }
            }
            if(balls[o].xVelocity < 0) {
                if(balls[o].xVelocity > -16) {
